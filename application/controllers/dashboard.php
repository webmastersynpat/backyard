						...
						for($t=0;$t<count($emailTo);$t++){
							if(!empty($emailTo[$t])){
								if((isset($pID) && $emailTo[$t]!=$pID) || isset($event['contact_id']) && $event['contact_id']!=$emailTo[$t]){
									$getContactDetail = $this->lead_model->getContactByEmail($emailTo[$t]);
