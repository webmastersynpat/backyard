...
?> 
<script>
__mainEntityFollow= 0;
__mainEntityType= "<?php echo $checkURIController;?>";
<?php 
	if(!empty($checkLead)):
?>
__mainEntityFollow = <?php echo $checkLead;?>;
<?php endif;?>
</script>
            ...
            <div class="panel-body">
                ...
                <div class="example-box-wrapper">
                    <div class="timeline-box timeline-box-left">
						<script>
							__timeline<?php echo $this->session->userdata['id']?> = [];
						</script>
						<?php 
						...
						?>
						<script>
						__timeline<?php echo $this->session->userdata['id']?>.push({'start':new Date('<?php echo $timeline->create_date?>'),'content': '<span class="tl-label bs-label <?php echo $colorClass;?>"><?php echo $label;?></span><span class="todo-content"><?php echo $timeline->message;?></span>'});
						...
						?>
							...
							jQuery(document).ready(function(){
								var items = new vis.DataSet(__timeline<?php echo $this->session->userdata['id']?>);
...
<script>
	function timelineItemBindEvents() {
		$('.tl-row .popover-content').off('click').on('click', function() {
			$('#activityInfoModal').find('.modal-title').html("Lead Name: "+jQuery(this).find('.tl-label').html());
			_content = jQuery(this).find('.tl-content').html().split("by");
			$('#activityInfoModal').find('.modal-body').html(
				"<p>Activity: " + _content[0]+"</p>" + 
				"<p>By: "+jQuery(this).find('.tl-timeuser').first().html().split('&nbsp;&nbsp;&nbsp;')[1]+"</p>" + 
				"<p>Date: "+jQuery(this).find('.tl-timeuser').first().html().split('&nbsp;&nbsp;&nbsp;')[0]+"</p>");
