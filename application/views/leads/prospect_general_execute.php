...
_res = "";
	...
	function record(level){
		...
		jQuery.ajax({
			...
			data:{token:jQuery("#token").val(),level:level,type:'General',complete:2},
				...
				if(response.results.length>0){
					...
					_tr = '<div class="row">' +
								'<div class="col-xs-4">' +
									'<div class="form-group input-string-group bigmr">' +
										'<label class="control-label">Patent Owner:</label>' +
										'<input type="text" class="form-control" id="patentOwner" value="'+_data.plantiffs_name+'" readonly="readonly" style="width: 30px;">' +
									'</div>' +
									'<div class="form-group">' +
										'<label class="control-label">Address:</label>' +
