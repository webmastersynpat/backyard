		...
		jQuery.ajax({
			...
			success:function(data){
				...
				if(_d!=undefined  && _d.length>0){
					jQuery("#litigationWorksheetId").empty().append("<option va;ue=''>-- Select Worksheet --</option>");
					for(i=0;i<_d.length;i++){
						jQuery("#litigationWorksheetId").append("<option value='"+_d[i].id+"' data-href='"+_d[i].full+"'>"+_d[i].text+"</option>");
					}
				}
			}
		});
				...
				if(response.results.length>0){
					...
					if(_data.patent_data!=""){
                     ...
                     /* Show btn blinking dpend upon condition */
						...
						if(_data.seller_info==2){
							jQuery("#assign_task").html("Seller Info Done");
						...
						if(_data.send_proposal_letter==2){
							jQuery("#request_for_proposal").html("Proposal letter created");
							jQuery("#request_for_proposal").addClass("btn btn-mwidth");
						}
                        if(_data.create_patent_list==1){
                            jQuery("#create_patent_list").addClass("btn-blink");
						}
						if(_data.create_patent_list==2){
							jQuery("#create_patent_list").html("Patent Spreadsheet created");
						...
						if(_data.embed_code!=""){
							jQuery("#schedule1stCall").html(_data.detail[0].litigation.embed_code);
							jQuery("#schedule1stCall").addClass("btn btn-mwidth btn-blink");
						}
						if(_data.nda_term_sheet==1){
							jQuery("#ndaTermSheet").html("NDA and TermSheet created");
            ...
            <?php echo form_open('leads/litigation',array('class'=>'form-horizontal form-flat','role'=>'form','id'=>'formLitigation',"onsubmit"=>"return dataValidate();", 'style'=>'margin-bottom: 0;'));?>
				...
				<div class="row row-width mrg5T">
					...
					<div class="col-sm-4 col-md-4 col-xs-4" style="padding-right: 7px;">
						...
						<div class="col-sm-7 col-md-7 col-xs-7" style='height:189px;overflow-y:scroll;overflow-x:hidden;' id="litigation_doc_list">
						...
						<div style="margin-right:-2px;">
							...
							<div class="mrg5T">
								<div class="todo-list-custom fright">
				    				<div class="row">
				    					<div class="col-sm-12" id="assign_task"><a class="btn btn-default btn-mwidth" href="javascript:void(0);" onclick="assign_task();">Collect Seller's info</a></div>
				    				</div>
				    				<div class="row mrg5T">
				    					<div class="col-sm-12" id="request_for_proposal"><a class="btn btn-default btn-mwidth" href="javascript:void(0);" onclick="sendRequestForProposalLetter(1)">Send Proposal Letter</a></div>
				    				</div>
				    				<div class="row mrg5T">
				    					<div class="col-sm-12" id="create_patent_list"><a class="btn btn-default btn-mwidth" href="javascript:void(0);" onclick="spreadsheet_box();">Create Patent List</a></div>
				    				</div>
				    				<div class="row mrg5T">
				    					<div class="col-sm-12" id="forward_to_review"><a class="btn btn-default btn-mwidth" onclick="forward_to_review();" href="javascript:void(0);">Forward</a></div>
				    				</div>
				    				<div class="row mrg5T">
				    				...
				    				<div class="row mrg5T">
				    					<div class="col-sm-12" id="ndaTermSheet"><a class="btn btn-default btn-mwidth" href="javascript:void(0);" onclick="createPartNDATermsheet()">NDA + TermSheet</a></div>
				    				</div>
				    			</div>
				    		</div>
						</div>
				...
				<div class="row row-width mrg5T">
					...
					<div class="col-sm-4">
						<label class="control-label" for="marketProspectsName">Are there > 10 licenses? Who?</label>
						<?php echo form_textarea(array('name'=>'comment[comment1]','id'=>'commentComment1','class'=>'form-control','rows'=>4,'cols'=>15));?>	
					</div>
					<div class="col-sm-4">
						<label class="control-label" for="marketProspectsName">Will licenses want to pay the expected fee? Why?</label>
						<?php echo form_textarea(array('name'=>'comment[comment2]','id'=>'commentComment2','class'=>'form-control','rows'=>4,'cols'=>15));?>	
					</div>
					<div class="col-sm-4">
						<label class="control-label" for="marketProspectsName">Seller's concerns and General Observations</label>
						<?php echo form_textarea(array('name'=>'comment[comment3]','id'=>'commentComment3','class'=>'form-control','rows'=>4,'cols'=>15));?>	
					</div>
					<div class="col-width" style="width: 180px; padding-right: 0px;">
				...
				<div class="row row-width mrg10T">
					...
					<div class="col-width" style="width:382px;"></div>
				</div>
				<div class="row row-width mrg10T">
					<!--div class="col-xs-5">
        ...
        jQuery.ajax({
            ...
            success:function(response){
                ...
                if(obj.url != '' && obj.error==0){                    
                    ...
                    jQuery("#create_patent_list").html("SpreadSheet created");
                    jQuery("#create_patent_list").addClass("btn btn-mwidth btn-blink");
...
function assign_task(){
        ...
        var seller_info = jQuery("#litigationSellerInfo").val();
        if(seller_info == 1)
        {
            jQuery("#assign_task").addClass("btn-blink");
        }
        ...
        { 
            ...
            success:function(response){ 
                    jQuery("#litigationSellerInfo").val('2');
    ...
    function sendRequestForProposalLetter(type){
        ...
        jQuery("#litigationProposal_letter").val('1');
        if(lead_id > 0)
        {
            if(type == 1)
            {
            ...
            if(type == 2)
            {
        		...
        		data:{name:_names,type:'Litigation',"lead_id":lead_id,ds:1},
        		...
        		success:function(res){
                       ...
                       // alert(_res);
            			if(_res.link!=""){
                            jQuery("#marketProposal_letter").val('2');
                            jQuery("#request_for_proposal a").hide();
                            jQuery("#request_for_proposal").html("Proposal letter created");
