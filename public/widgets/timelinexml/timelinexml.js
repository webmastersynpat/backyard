(function($) {
	...
	Controller.prototype.setDOMEvents = function() {
		var root = this;
		$('.timeline-event-node').off('click').on('click', function() {
		...
		$('.timeline-event-node').off('mouseover').on('mouseover', function() {
		...
		$('.timeline-event-node').off('mouseout').on('mouseout', function() {
		...
		timelinexml : function(options) {
			...
			return this.each(function() {
				...
				if (settings.src.length != 0) {
					...
					$('.timeline-event-node').each(function(){
						view.showEvent($(this).closest('.timeline-event').attr('id'));
					});
